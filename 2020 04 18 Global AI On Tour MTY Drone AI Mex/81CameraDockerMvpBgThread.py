...
def processImage(imgNumber, img):
...
while True:
    ...
    try:
        ...
        # predSorted = getPredictionsSorted(jsonStr)
        # do some stuff
        download_thread = threading.Thread(target=processImage, args=i, img)
        ...
        for j, line in enumerate(frameInfo.split('\n')):
            j = j + 1
            cv2.putText(frame, line, (10, 10 * j), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
