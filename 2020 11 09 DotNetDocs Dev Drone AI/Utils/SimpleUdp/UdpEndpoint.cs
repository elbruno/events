...
namespace SimpleUdp
{
    ...
    public class UdpEndpoint
    {
        ...
        public UdpEndpoint(string ip, int port)
        {
            ...
            //var ipEp = new IPEndPoint(_IPAddress, port);
            //_Socket.Bind(ipEp);
        ...
        public void StartServer()
        {
            ...
            Debug.WriteLine($"Start Server for {_Endpoint.ToString()}- MaxDataGramSize:{_MaxDatagramSize}");
        ...
        public void Receive()
        {
            ...
            _Socket.BeginReceiveFrom(state.Buffer, 0, _MaxDatagramSize, SocketFlags.None, ref _Endpoint, _ReceiveCallback = (ar) =>
            {
                byte[] buffer = new byte[_MaxDatagramSize];
                Buffer.BlockCopy(state.Buffer, 0, buffer, 0, _MaxDatagramSize);
                DatagramReceived?.Invoke(this, new Datagram(_Ip, _Port, buffer));
            }, state);
        }
        ...
        {
            Debug.WriteLine($"Send message: {_Ip}, {_Port}, {text}"  );
            Send(_Ip, _Port, text);
        }
