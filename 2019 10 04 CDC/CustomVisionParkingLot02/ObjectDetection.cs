...
namespace CustomVisionParkingLot02
{
    ...
    public sealed class PredictionModel
    {
        ...
        {
            return $"{TagName} {Probability * 100.0f:#0.00}%{Environment.NewLine}\tOriginal\tTop: {BoundingBox.Top}, Left: {BoundingBox.Left}, Height: {BoundingBox.Height}, Width: {BoundingBox.Width}";
        }
    }
    ...
    public class ObjectDetection
    {
        ...
        private List<PredictionModel> SuppressNonMaximum(ExtractedBoxes extractedBoxes)
        {
            ...
            if (extractedBoxes.Probabilities.Count > 0)
            {
                ...
                while (predictions.Count < MaxDetections)
                {
                    ...
                    {
                        if (CalculateIou(extractedBoxes.Boxes[index], extractedBoxes.Boxes[i]) > IouThreshold)
                        {
        ...
        {
            var extractedBoxes = ExtractBoxes(predictionOutputs, Anchors);
