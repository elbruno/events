...
def initialize():
    ...
    print(f'Network input size: {network_input_size}')
...
def extract_and_resize_to_256_square(image):
    ...
    if use_opencv:
        log_msg("call cv2.resize")
        min_size = min(h, w)
        image = crop_center(image, min_size, min_size)
        return cv2.resize(image, (256, 256), interpolation = cv2.INTER_LINEAR)
    else:
        log_msg("call extract_and_resize from extract_and_resize_to_256_square")
        return extract_and_resize(image, (256, 256))

def extract_and_resize_to_network_input_size_square(image):
    """
    extracts image central square crop and resizes it to 256x256
    image: input image numpy array
    returns resized 256x256 central crop as numpy array
    """
    h, w = image.shape[:2]
    log_msg("crop_center: " + str(w) + "x" + str(h) +" and resize to " + str(network_input_size) + "x" + str(network_input_size))
    if use_opencv:
        log_msg("call cv2.resize")
        min_size = min(h, w)
        image = crop_center(image, min_size, min_size)
        return cv2.resize(image, (network_input_size, network_input_size), interpolation = cv2.INTER_LINEAR)
    else:
        log_msg("call extract_and_resize from extract_and_resize_to_network_input_size _square")
        return extract_and_resize(image, (network_input_size, network_input_size))
...
def preprocess_image(image_pil):
    ...
    resized_image = extract_and_resize_to_network_input_size_square(image)
...
def predict_image(image):
    ...
    try:
        ...
        log_msg("image cropped")

        tf.compat.v1.reset_default_graph()
        tf.import_graph_def(graph_def, name='')

        log_msg("tf compat and import done")
