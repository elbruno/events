...
namespace ElBruno.ProjectOxford.FaceApiEmotionVisionSample.Lib
{
    internal class ProjectOxfordHelper
    {
        ...
        public async Task<Tuple<ObservableCollection<Face>, ObservableCollection<Face>>> StartFaceDetection(string selectedFile, bool analyzeEmotion)
        {
            ...
            using (var fileStreamImage = File.OpenRead(selectedFile))
            {
                try
                {
                    ...
                    var faces = await client.DetectAsync(fileStreamImage, false, true, faceAttributes);
                    ...
                    foreach (var face in faces)
                    {
                        var detectedFace = new Face()
                        {
                            ...
                            Age = $"{face.FaceAttributes.Age:#} years old",
                        };
        ...
        public async Task<AnalysisResult> AnalyzeImage(string selectedFile)
        {
            ...
            try
            {
                VisualFeature[] visualFeatures = new[] { VisualFeature.Adult, VisualFeature.Categories, VisualFeature.Color, VisualFeature.Description, VisualFeature.Faces, VisualFeature.ImageType, VisualFeature.Tags };
                ...
                if (File.Exists(selectedFile))
                {
                    ...
                    {
                        analysisResult = await visionClient.AnalyzeImageAsync(fileStreamVision, visualFeatures);
                    }
                }
                else if (Uri.IsWellFormedUriString(selectedFile, UriKind.RelativeOrAbsolute))
                {
                    analysisResult = await visionClient.AnalyzeImageAsync(selectedFile, visualFeatures);
                }
                else
                {
        ...
        public async Task<string> AnalyzeImageAsString(string imagePathOrUrl)
        {
            ...
            return await AnalyzeImageAsString(result);
        }

        public async Task<string> AnalyzeImageAsString(AnalysisResult result)
        {
