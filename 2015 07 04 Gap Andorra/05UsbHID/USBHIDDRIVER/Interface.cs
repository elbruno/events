...
namespace USBHIDDRIVER
{
    ...
    public class USBInterface
    {
        ...
        private USB.HIDUSBDevice _usbdevice;

        public USB.HIDUSBDevice UsbDevice
        {
       ...
       public  USBInterface(string vid, string pid)
        {
            this.usbVID = vid;
            this.usbPID = pid;
            this._usbdevice = new USB.HIDUSBDevice(this.usbVID, this.usbPID);
        }
        ...
        {
            this.usbVID = vid;
            this._usbdevice = new USB.HIDUSBDevice(this.usbVID, "");
        }
        ...
        public bool Connect()
        {
            _isConnected = this._usbdevice.connectDevice();
            return _isConnected;
        }
        ...
        public void Disconnect()
        {
            if (_isConnected)
            {
                this._usbdevice.disconnectDevice();
            }
        }
        ...
        {
            return (String[])this._usbdevice.getDevices().ToArray(typeof(string));
        }
        ...
        public bool write(Byte[] bytes)
        {
                ...
                while (bytePos <= byteCount-1)
                {
                    ...
                    //send the report
                    if (!this._usbdevice.writeData(transfByte))
                    {
                        success = false;
                    }
                    Thread.Sleep(5);
